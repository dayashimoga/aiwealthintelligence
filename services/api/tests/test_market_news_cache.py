...
from app.infrastructure.market.market_data_service import YFinanceMarketDataService, _format_symbol
from app.infrastructure.market.price_cache import get_cached_price, set_cached_price
from app.infrastructure.market.news_fetcher import fetch_rss_news, analyze_news_sentiment
...
async def test_price_cache() -> None:
    ...
    with patch("app.infrastructure.market.price_cache.cache_repo", new_callable=AsyncMock) as mock_cache:
...
async def test_analyze_news_sentiment() -> None:
    ...
    with patch("app.infrastructure.market.news_fetcher.get_ai_provider", return_value=mock_provider):
        analysis = await analyze_news_sentiment("TCS Q3 Results", "TCS reports 15% growth in net profit.")
