...
class TestAASecurityService:
    ...
    def test_key_generation_and_serialization(self) -> None:
        ...
        assert (
            len(public_bytes) == 65
        )  # Uncompressed NIST P-256 public keys are exactly 65 bytes (0x04 + 32-byte X + 32-byte Y)
    ...
    def test_key_exchange_and_aes_decryption(self) -> None:
        ...
        plain_text = (
            "<equity><isin>INE002A01018</isin><units>10.0</units><rate>2400.0</rate></equity>"
        )
        import os
