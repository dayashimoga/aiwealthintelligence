...
from app.infrastructure.services.setu_aa_service import AASecurityService, setu_aa_service
...
async def consent_callback(
    ...
    if not is_sandbox and setu_aa_service.is_configured:
        try:
            ...
            fetched_holdings = await setu_aa_service.fetch_financial_data(
                consent_id, private_key, public_bytes
            )
            if fetched_holdings:
    ...
    logger.info(
        "aa_consent_completed", consent_handle=consent_handle, imported=len(holdings_to_create)
    )
